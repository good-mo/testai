<template>
  ...
  >
    ...
    <template #target-title="{ countTotal, checked, indeterminate, onSelectAllChange }">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-[8px]">
          <a-checkbox :model-value="checked" :indeterminate="indeterminate" @change="onSelectAllChange" />
          {{ t('ms.transfer.selected', { count: countTotal }) }}
        </div>
        <MsButton type="text" :disabled="countTotal === 0" @click="clearTarget">{{ t('common.clear') }}</MsButton>
      </div>
    </template>
...
<script setup lang="ts">
   ...
   * 清空右侧已选。
   *
   * arco 的 target-title 插槽并不提供 onClear，此前直接把插槽解构的 onClear
   * 绑到「清空」按钮上，实际拿到的是 undefined —— 按钮点了没反应。
   * 这里改为自己清空 innerTarget，由 watch 同步给 v-model。
   */
