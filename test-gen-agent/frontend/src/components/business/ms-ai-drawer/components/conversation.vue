...
<script setup lang="ts">
  ...
  async function handleSaveTitle() {
    try {
      ...
      Message.error(t('ms.ai.titleUpdateFailed'));
    }
  }
  ...
  async function syncSingleApiCase() {
    try {
      ...
      Message.error(t('ms.ai.syncApiCaseFailed'));
    } finally {
      loading.value = false;
    }
  }
  ...
  async function syncSingleFeatureCase() {
    try {
      ...
      Message.error(t('ms.ai.syncFeatureCaseFailed'));
    } finally {
      loading.value = false;
    }
  }
  ...
  async function batchSaveAiCase() {
    try {
      ...
      Message.error(t('ms.ai.batchSaveFailed'));
    } finally {
      loading.value = false;
    }
  }
  ...
  async function initDetail() {
    ...
    if (activeConversation.value) {
      try {
        ...
        Message.error(t('ms.ai.loadDetailFailed'));
        conversationItems.value = [];
        checkedCases.value = [];
      } finally {
        loading.value = false;
