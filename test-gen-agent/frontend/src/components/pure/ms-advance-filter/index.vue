...
<script setup lang="ts">
  ...
  async function getUserViewList() {
    try {
      ...
      const res = (await getViewList(props.viewType as ViewTypeEnum, appStore.currentProjectId)) as any;
      // 兼容后端可能返回空数组或字段缺失的情况
      const data = res && typeof res === 'object' && !Array.isArray(res) ? res : {};
      internalViews.value = Array.isArray(data?.internalViews) ? data.internalViews : [];
      customViews.value = Array.isArray(data?.customViews) ? data.customViews : [];
    } catch (error) {
  ...
  async function getMemberOptions() {
    try {
      ...
      memberOptions.value = [{ name: t('common.currentUser'), id: 'CURRENT_USER' }, ...memberList].map(
        (e: any) => ({
          label: e.name || e.username || e.id,
          value: e.id || e.value,
        })
      );
    } catch (error) {
      // eslint-disable-next-line no-console
      console.log(error);
      memberOptions.value = [{ name: t('common.currentUser'), id: 'CURRENT_USER' }];
    }
  }
  ...
  onMounted(async () => {
    if (props.viewType) {
      ...
      const firstInternalId = internalViews.value[0]?.id;
      if (props.viewName) {
        currentView.value = props.viewName;
        isAdvancedSearchMode.value = currentView.value !== firstInternalId;
      } else {
        currentView.value = firstInternalId;
      }
    }
  });
