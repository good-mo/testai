...
<script lang="ts">
  ...
  // 仅引入类型，运行时 monaco 采用懒加载，避免 codeEditor chunk 同步进入首屏
  ...
  /**
   * 懒加载 monaco-editor：仅在编辑器组件首次挂载初始化时才加载 monaco 大体积包。
   * 首屏/路由切换时不会同步拉取 monaco，从而降低首屏体积。
   */
  function loadMonaco(): Promise<MonacoApi> {
    if (!monacoPromise) {
      monacoPromise = (async () => {
        // 先初始化 monaco 的 worker 环境（内部会按需 import monaco-editor）
  ...
  export default defineComponent({
    ...
    setup(props, { emit, slots }) {
      ...
      async function handleLanguageChange(val: Language) {
        ...
        monacoApi.editor.setModelLanguage(editor.getModel()!, val.toLowerCase());
      }
      ...
      const init = async () => {
        ...
        //   monacoApi.editor.defineTheme(e, MsCodeEditorTheme[e as CustomTheme]);
        // });
        editor = monacoApi.editor.create(codeContainerRef.value, {
        ...
        editor.getModel()?.setEOL(monacoApi.editor.EndOfLineSequence.LF); // 设置换行符
      ...
      // 初始化diffEditor
      const initDiffEditor = async (originalValue: string, modifiedValue: string) => {
        const monacoApi = await loadMonaco();
        diffEditor = monacoApi.editor.createDiffEditor(codeContainerRef.value, {
          automaticLayout: true,
          padding: {
            top: 12,
            bottom: 12,
        ...
        const originalModel = monacoApi.editor.createModel(originalValue, props.language.toLowerCase());
        const modifiedModel = monacoApi.editor.createModel(modifiedValue, props.language.toLowerCase());
      ...
      watch(
        ...
        async (newValue) => {
          ...
          monacoApi.editor.setModelLanguage(editor.getModel()!, newValue.toLowerCase()); // 设置语言，语言 ENUM 是大写的，但是 monaco 需要小写
          // 设置对比初始和修改值
          if (diffEditor) {
            const originalModel = diffEditor.getModel()?.original;
            const modifiedModel = diffEditor.getModel()?.modified;
            if (originalModel && modifiedModel) {
              monacoApi.editor.setModelLanguage(originalModel, newValue.toLowerCase());
              monacoApi.editor.setModelLanguage(modifiedModel, newValue.toLowerCase());
            }
          }
        }
      );
      ...
      onMounted(async () => {
        if (props.diffMode) {
          await initDiffEditor(props.originalValue, props.modelValue);
        } else {
          await init();
        }
        setEditBoxBg();
