...
<script setup lang="ts">
  ...
  // 注意：不能在 <script setup> 顶层 await initColumns()，
  // 否则 setup 会变成 async，组件需被 <Suspense> 包裹才能挂载；
  // 接口调试/调试页等使用该表单表格的地方均无 <Suspense> 边界，会导致
  // "setup function returned a promise" 报错。故将列注册移入 onBeforeMount，
  // 在子组件（MsBaseTable）挂载前完成列配置注册。
  onBeforeMount(async () => {
    await initColumns();
  });
</script>
