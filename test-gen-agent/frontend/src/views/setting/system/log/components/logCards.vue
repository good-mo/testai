...
<script lang="tsx" setup>
  ...
  function searchLog() {
    ...
    // 项目日志页面：自动限定到当前项目；组织日志页面：默认限定当前组织
    if (props.mode === MENU_LEVEL[2]) {
      projectIds = appStore.currentProjectId ? [appStore.currentProjectId] : [];
    } else if (props.mode === MENU_LEVEL[1]) {
      organizationIds = appStore.currentOrgId ? [appStore.currentOrgId] : [];
    } else if (!MENU_LEVEL.includes(level.value) && typeof operateRange.value[0] === 'object') {
  ...
  // 否则 setup 会变成 async，组件需被 <Suspense> 包裹才能挂载；
  // 本页路由为懒加载且无 Suspense 边界时组件无法挂载，
  // 列表请求（/project/log/list）不会发出，界面无任何返回。
  // 故将列注册移入 onBeforeMount 中，先注册列配置再加载列表数据。
  onBeforeMount(async () => {
