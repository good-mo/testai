...
<script lang="ts" setup>
  ...
  const saveCallBack = async (isNew = false) => {
    // 校验通过回调保存
    loading.value = true;
    // 确保表单最新值已同步到 store 后再提交，避免 blur 时序问题
    store.currentEnvDetailInfo.name = form.name;
    store.currentEnvDetailInfo.description = form.description as string;
      ...
      if (!valid) {
        try {
          ...
          if (activeKey.value === EnvTabTypeEnum.ENVIRONMENT_HOST) {
            hostTabRef.value?.validateForm(() => saveCallBack(false));
          } else {
            envKeyword.value = '';
            await nextTick();
            await saveCallBack();
  ...
  // 注意：不能在 <script setup> 顶层 await initPlugin()/initContentTabList()/getContentTabList()，
  // 否则 setup 会变成 async，组件需被 <Suspense> 边界包裹才能挂载；
  // 本页面渲染链路没有 <Suspense>，会触发
  // "setup function returned a promise, but no <Suspense> boundary was found..."
  // 的 Vue 警告且组件无法正常渲染。故将插件与 Tab 初始化封装成 initEnvContent，放到 onMounted 中执行。
  async function initEnvContent() {
    await initPlugin();
