...
<script setup lang="ts">
  ...
  async function changeMessageReceivers(val: boolean, record: TableMessageChildrenItem, _dataIndex: string) {
    if (!val) {
      if (record.receivers.length === 0) {
      ...
      try {
        record.loading = true;
        const receiverIds = record.receivers.map((e: Receiver) => (e.id !== undefined ? e.id : e));
        // 接收人在前端是事件级共享的（同事件所有机器人共用），
        // 但后端按 (task_type,event,robot_id) 存储，因此将接收人同步到该事件下所有机器人配置。
        const robotConfigs = Object.values(record.projectRobotConfigMap || {});
        const savePromises = robotConfigs.map((robotConfig: ProjectRobotConfig) =>
          saveMessageConfig({
            ...robotConfig,
            projectId: appStore.currentProjectId,
            taskType: record.taskType,
