...
<script setup lang="ts">
  ...
  // 本页面渲染链路无 <Suspense> 边界，组件将无法挂载、文件列表请求不发出、界面无数据。
  // 故将列初始化移入下方 onMounted 中执行，先注册列配置再拉取列表。
  ...
  onMounted(() => {
    // 列初始化不能在 <script setup> 顶层 await，否则 setup 变 async、组件无法在无 <Suspense> 边界下挂载，
    // 导致文件列表无数据。放到挂载期统一初始化列配置。
    tableStore.initColumn(TableKeyEnum.FILE_MANAGEMENT_FILE, columns, 'drawer', true);
