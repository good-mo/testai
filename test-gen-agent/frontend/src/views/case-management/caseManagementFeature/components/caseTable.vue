...
<script setup lang="ts">
  ...
  // 默认字段是否已初始化（列配置需在首次拉取列表前完成注册）
  let defaultFieldsReady = false;
  async function ensureDefaultFields() {
    if (defaultFieldsReady) return;
    await getDefaultFields();
    defaultFieldsReady = true;
  }
  ...
  async function mountedLoad() {
    ...
    // 首次加载前先注册默认字段/自定义列到 IndexedDB。
    // 注意：不能在 <script setup> 顶层 await getDefaultFields()，否则 setup 变 async，
    // 组件需被 <Suspense> 包裹才能挂载；本页无 Suspense 边界时组件无法挂载，
    // 列表请求（/functional/case/page）不会发出、右侧面板无数据。
