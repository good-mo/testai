<template>
  <div class="condition-status" :style="getClass"> {{ t(getStatusInfo.label) }} </div>
</template>
...
<script setup lang="ts">
  ...
  // 兜底：未知/空步骤类型统一按 API 接口展示，避免渲染崩溃
  const getStatusInfo = computed(
    () => scenarioStepMap[props.status] || { label: 'report.detail.api', color: 'rgb(var(--link-4))' }
  );
  ...
  const getClass = computed(() => {
    if (props.status) {
      return {
        ...
        border: `1px solid ${getStatusInfo.value.color}`,
      };
    }
  });
</script>
