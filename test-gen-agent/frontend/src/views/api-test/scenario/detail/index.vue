      ...
      <a-tabs v-model:active-key="activeKey" class="h-full" animation lazy-load>
        ...
        >
          <params
            ...
            v-model:common-variables="scenario.scenarioConfig.variable.commonVariables"
            v-model:csv-variables="scenario.scenarioConfig.variable.csvVariables"
            @change="scenario.unSaved = true"
          />
        </a-tab-pane>
        <a-tab-pane
...
<script setup lang="ts">
  ...
  async function toggleFollowReview() {
    try {
      ...
      // 单端点 toggle 后回读服务器侧实际关注状态，避免纯乐观翻转造成状态漂移
      const res = await followScenario<{ followed?: boolean }>(scenario.value.id || '');
      scenario.value.follow = typeof res?.followed === 'boolean' ? res.followed : !scenario.value.follow;
