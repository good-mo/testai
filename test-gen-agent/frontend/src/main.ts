...
async function bootstrap() {
  ...
  app.use(store);
  // 默认语言获取为网络请求，与本地语言包加载互不依赖，先行并行发起，避免串行拉长首屏时间
  const localLocale = localStorage.getItem('MS-locale');
  const defaultLocalePromise = localLocale
    ? Promise.resolve(null)
    : getDefaultLocale().catch(() => null);

  // 注册国际化，需要异步阻塞，确保语言包加载完毕（本地语言包加载，非网络阻塞）
  await setupI18n(app);
  // 等待默认语言网络请求结果并应用
  const defaultLocale = await defaultLocalePromise;
  if (defaultLocale) {
