...
<script lang="ts" setup>
  ...
  onBeforeMount(async () => {
    // 公钥获取与登录态检查互不依赖，先行并行发起，缩短串行阻塞时间
    const publicKeyPromise = getPublicKey().catch(() => null);
    ...
    // 登录态等网络请求并行发起期间公钥通常已返回，此处等待不会产生额外阻塞；公钥失败不影响后续初始化
  ...
  onMounted(() => {
    window.onerror = (message, source, _lineno, _colno, error) => {
      ...
      if (msg.includes('Failed to fetch dynamically imported')) {
        showUpdateMessage();
        return false;
      }
      // 抑制第三方性能监控库 web-vitals 在 SPA 路由切换时的已知 bug 报错，
      // 该库不在本应用代码中，属浏览器/环境注入的 RUM 脚本，不影响页面功能。
      if (
        (msg.includes('Cannot read properties of undefined') || msg.includes("reading 'startTime'")) &&
        /reportAllChanges/.test(String(source || '') + String(error?.stack || ''))
      ) {
        return true;
      }
      return false;
