<template>
  <div :class="['ms-base-table', $slots.quickCreate ? 'ms-base-table--hasQuickCreate' : '']">
    ...
    <a-table
      ...
      :virtual-list-props="autoVirtualListProps"
...
<script lang="ts" setup>
  ...
  // 计算有效 scroll（结合 attrs 与 scrollObj 覆盖）
  const effectiveScroll = computed(() => {
    const attrsScroll = (attrs.scroll || {}) as Record<string, any>;
    const scrollOverride = (scrollObj.value.scroll || {}) as Record<string, any>;
    return { ...attrsScroll, ...scrollOverride };
  });

  /**
   * 数据是否包含树形结构（记录带 children 子节点）。
   * 树形表格展开/折叠时行高会变化，与虚拟滚动的固定行高假设冲突，应跳过虚拟滚动。
   */
  const hasTreeChildren = computed(() => {
    const data = (attrs.data || []) as Record<string, any>[];
    // 顶层即包含带 children 的记录（可能为懒加载树形结构）
    if (data.some((row) => Array.isArray(row?.children) && row.children.length > 0)) {
      return true;
    }
    // 展开的行记录包含 children 时也应跳过（避免展开时行错位）
    const expanded = expandedKeys.value || [];
    return data.some((row) => expanded.includes(row[rowKey || 'id']) && Array.isArray(row?.children));
  });

  /**
   * 自动启用虚拟滚动：当 scroll.y 为正数时开启，支持通过 virtualListProps 显式覆盖/禁用。
   * 表格使用 expand（行高可变）、spanMethod（行合并）或树形 children 数据等特性时不兼容虚拟滚动。
   */
  const autoVirtualListProps = computed(() => {
    // 用户显式传入了 virtual-list-props（含 null/false 禁用、对象自定义配置）
    if (attrs.virtualListProps !== undefined) {
      return attrs.virtualListProps || undefined;
    }
    // 展开行/行合并/树形结构等特性与虚拟滚动不兼容时跳过
    if (attrs.showExpand || attrs.showFirstOperation || props.spanMethod || hasTreeChildren.value) {
      return undefined;
    }
    // 仅当 scroll.y 为有限正数时启用（字符串如 '100%' 或空值时不可用）
    const y = effectiveScroll.value.y;
    if (typeof y !== 'number' || !Number.isFinite(y) || y <= 0) {
      return undefined;
    }
    return {
      height: y,
      threshold: 100, // 超过 100 行才启用虚拟化
    };
  });
