...
import type { FilterResult } from '../ms-advance-filter/type';
...
export default function useTableProps<T>(
  ...
  // 属性组
  const propsRes = ref<MsTableProps<T>>(cloneDeep(defaultProps)) as Ref<MsTableProps<T>>;
    ...
    if (propsRes.value.showPagination) {
      ...
      try {
        if (loadListFunc) {
          ...
          setTableErrorStatus(false);
          propsRes.value.data = dataArr.map((item: MsTableDataItem<T>) => {
            if (item.updateTime) {
              item.updateTime = dayjs(item.updateTime).format('YYYY-MM-DD HH:mm:ss');
            }
            if (item.createTime) {
