...
<script lang="ts" async setup>
  ...
  // 首次进入前需先注册默认列/筛选项到表配置。
  // 注意：不能在 <script setup> 顶层 await 上述初始化，否则 setup 会变成 async，
  // 组件需被 <Suspense> 包裹才能挂载；本页无 Suspense 边界时组件无法挂载，
  // 回收站列表请求（/bug/trash/page）不会发出，列表无数据。
  ...
  onMounted(async () => {
    ...
    try {
      ...
      // 列头/筛选项初始化失败也不能阻断列表请求，否则回收站(/bug/trash/page)不会发出、界面无数据
      fetchData();
    }
  });
</script>
