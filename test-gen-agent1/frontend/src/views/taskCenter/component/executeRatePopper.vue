    ...
    <template #content>
      <a-spin :loading="record.loading" class="flex w-[130px] flex-col gap-[8px]">
        ...
        <div class="ms-taskCenter-execute-rate-item">
          <div class="ms-taskCenter-execute-rate-item-label">
            <div
              class="ms-taskCenter-execute-rate-item-label-point" :style="{ backgroundColor: executeFinishedRateMap.UN_EXECUTE.color }"
            ></div>
        ...
        <div class="ms-taskCenter-execute-rate-item">
          <div class="ms-taskCenter-execute-rate-item-label">
            <div
              class="ms-taskCenter-execute-rate-item-label-point" :style="{ backgroundColor: executeFinishedRateMap.SUCCESS.color }"
            ></div>
        ...
        <div class="ms-taskCenter-execute-rate-item">
          <div class="ms-taskCenter-execute-rate-item-label">
            <div
              class="ms-taskCenter-execute-rate-item-label-point" :style="{ backgroundColor: executeFinishedRateMap.FAKE_ERROR.color }"
            ></div>
        ...
        <div class="ms-taskCenter-execute-rate-item">
          <div class="ms-taskCenter-execute-rate-item-label">
            <div
              class="ms-taskCenter-execute-rate-item-label-point" :style="{ backgroundColor: executeFinishedRateMap.ERROR.color }"
            ></div>
  ...
  async function handleExecuteRatePopVisibleChange(_visible: boolean) {
    if (_visible && props.executeTaskStatisticsRequest) {
      try {
        ...
        const stat = Array.isArray(res) && res.length > 0 ? res[0] : undefined;
        if (stat) {
          record.value.executeRate = stat.executeRate;
          record.value.pendingCount = stat.pendingCount;
          record.value.successCount = stat.successCount;
          record.value.fakeErrorCount = stat.fakeErrorCount;
          record.value.errorCount = stat.errorCount;
          record.value.caseTotal = stat.caseTotal;
        }
      } catch (error) {
