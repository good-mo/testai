<template>
  ...
  >
    ...
    <template #resourceType="{ record }">
      {{ t(scheduleTaskTypeMap[record.resourceType] || 'common.unKnow') }}
    </template>
    <template #[FilterSlotNameEnum.GLOBAL_TASK_CENTER_SYSTEM_TASK_TYPE]="{ filterContent }">
      {{ t(scheduleTaskTypeMap[Array.isArray(filterContent.value) ? filterContent.value[0] : filterContent.value] || 'common.unKnow') }}
    </template>
