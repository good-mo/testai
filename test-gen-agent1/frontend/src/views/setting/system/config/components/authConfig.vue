        ...
        <template v-else-if="activeAuthForm.type === 'OIDC'">
          ...
          >
            <a-input
              ...
              :placeholder="
                t('system.config.auth.commonUrlPlaceholder', {
                  url: 'http://<keyclock>auth/realms/<testpilot>/protocol/openid-connect/auth',
                })
              "
              allow-clear
            ></a-input>
          ...
          >
            <a-input
              ...
              :placeholder="
                t('system.config.auth.commonUrlPlaceholder', {
                  url: 'http://<keyclock>auth/realms/<testpilot>/protocol/openid-connect/token',
                })
              "
              allow-clear
            ></a-input>
          ...
          >
            <a-input
              ...
              :placeholder="
                t('system.config.auth.commonUrlPlaceholder', {
                  url: 'http://<keyclock>auth/realms/<testpilot>/protocol/openid-connect/userinfo',
                })
              "
              allow-clear
            ></a-input>
          ...
          >
            <a-input
              ...
              :placeholder="
                t('system.config.auth.commonUrlPlaceholder', {
                  url: 'http://<testpilot-endpoint>/sso/callback or http://<testpilot-endpoint>/sso/callback/authld',
                })
              "
              allow-clear
            ></a-input>
          ...
          >
            <a-input
              ...
              :placeholder="
                t('system.config.auth.commonUrlPlaceholder', {
                  url: 'http://<keyclock>/auth/realms/<testpilot>/protocol/openid-connect/logout',
                })
              "
              allow-clear
            ></a-input>
...
<script setup lang="ts">
  ...
  onBeforeMount(async () => {
    // 注意：不能在 <script setup> 顶层 await tableStore.initColumn，
    // 否则 setup 会变成 async，无 <Suspense> 边界时组件无法挂载。
