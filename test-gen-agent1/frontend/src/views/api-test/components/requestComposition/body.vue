<template>
  ...
  <a-spin v-else :loading="bodyLoading" class="block h-[calc(100%-34px)]">
    ...
    <MsJsonSchema
      ...
      v-model:selected-keys="innerParams.jsonBody.jsonSchemaTableSelectedRowKeys"
      :disabled="props.disabledExceptParam"
      @change="() => emit('change')"
    />
    <MsCodeEditor
...
<script setup lang="ts">
  ...
  /**
   * 补齐 body 中可能缺失的子对象结构
   * 后端返回的调试详情/接口详情中 body 字段可能不完整，需要兜底避免访问 undefined 属性报错
   */
  function ensureBodyStructure() {
    const body = innerParams.value;
    if (!body) return;
    if (body.bodyType === undefined) {
      body.bodyType = RequestBodyFormat.NONE;
    }
    if (!body.formDataBody) {
      body.formDataBody = { formValues: [] };
    }
    if (!body.wwwFormBody) {
      body.wwwFormBody = { formValues: [] };
    }
    if (!body.jsonBody) {
      body.jsonBody = {
        jsonValue: '',
  ...
  watch(
    ...
    () => {
      const formValues = innerParams.value.formDataBody?.formValues || [];
      if (formValues.length > 0) {
        ...
        if (hasNullFiles && innerParams.value.formDataBody) {
