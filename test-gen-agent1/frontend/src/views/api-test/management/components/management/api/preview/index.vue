...
<script setup lang="ts">
  ...
  async function toggleFollowReview() {
    try {
      ...
      const res = await toggleFollowDefinition<{ followed?: boolean }>(previewDetail.value.id);
      previewDetail.value.follow = typeof res?.followed === 'boolean' ? res.followed : !previewDetail.value.follow;
      Message.success(previewDetail.value.follow ? t('common.followSuccess') : t('common.unFollowSuccess'));
      emit('updateFollow');
    } catch (error) {
