<template>
  ...
  >
    <div class="preview-rounded h-full overflow-scroll bg-[var(--color-text-fff)]">
      ...
      <div class="flex gap-[12px] p-[16px]">
        <a-avatar :image-url="platformLogo" />
        <div class="flex flex-1 flex-col overflow-hidden">
          <div class="font-medium text-[var(--color-text-1)]" v-dompurify-html="subject || '-'"></div>
          <div class="my-[4px] flex-1 text-[var(--color-text-2)]" v-dompurify-html="template || '-'"></div>
  ...
  >
    <div class="mb-[4px] text-[16px] font-medium leading-[24px] text-[var(--color-text-1)]" v-dompurify-html="subject || '-'">
    </div>
    ...
    <div class="preview-rounded bg-[var(--color-text-fff)]">
      ...
      <div class="flex flex-col px-[16px] pb-[16px]">
        ...
        <div class="relative rounded-[var(--border-radius-small)] bg-[var(--color-text-n9)] p-[12px]">
          ...
          <div class="text-[var(--color-text-2)]" v-dompurify-html="`【${subject}】${template || '-'}`"></div>
        </div>
      </div>
    </div>
  </div>
  ...
  >
    ...
    <div class="flex flex-col px-[12px] pb-[12px]">
      ...
      <div
        ...
        v-dompurify-html="`【${subject}】${template || '-'}`"
      >
      </div>
    </div>
  </div>
...
<script setup lang="ts">
  ...
  const platformLogo = `${import.meta.env.BASE_URL}images/TestPilot-logo.svg`;
  ...
  const subject = computed(() => {
    ...
    if (
      ...
      props.robot.previewSubject?.indexOf('TestPilot') === -1
    ) {
      previewName = `TestPilot ${previewName}`;
    }
    return previewName;
  });
