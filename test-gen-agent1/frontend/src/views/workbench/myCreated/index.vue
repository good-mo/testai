...
<script setup lang="ts">
  ...
  async function handleProjectSelect(val?: string, _project?: ProjectListItem) {
    if (_project) {
      // moduleSetting 为 JSON 数组字符串；后端可能返回空串导致 JSON.parse 抛错，需容错处理
      let _currentProjectFeatures: string[] = [];
      try {
        const _parsed = JSON.parse(_project.moduleSetting || '[]');
        _currentProjectFeatures = Array.isArray(_parsed) ? _parsed : [];
      } catch {
        _currentProjectFeatures = [];
      }
      // 解析结果为空时回退到项目的启用模块（moduleIds），保证各模块列表正常展示
      if (_currentProjectFeatures.length === 0) {
        _currentProjectFeatures = _project.moduleIds || [];
      }
