...
<script setup lang="ts">
  ...
  // 注意：不能在此处 <script setup> 顶层 await tableStore.initColumn，
  // 否则 setup 会变成 async，组件需被 <Suspense> 包裹才能挂载；
  // 而无 Suspense 时组件无法挂载，报告列表请求（/test-plan/report/page）不会发出。
  // 故将其移到下方 onBeforeMount 中执行。
  ...
  onBeforeMount(() => {
    tableStore.initColumn(TableKeyEnum.TEST_PLAN_REPORT_TABLE, columns, 'drawer');
    if (route.query.id) {
