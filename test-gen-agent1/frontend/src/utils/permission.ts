...
/**
 * 判断权限列表中是否包含指定权限。
 * 兼容后端返回的通配符权限 `*`（表示拥有全部权限）。
 */
export function hasSpecificPermission(permissionList: string[], permission: string) {
...
export function hasPermission(permission: string, typeList: string[]) {
  ...
  if (typeList.includes('PROJECT') && hasSpecificPermission(projectPermissions, permission)) {
    return true;
  }
  if (typeList.includes('ORGANIZATION') && hasSpecificPermission(orgPermissions, permission)) {
    return true;
  }
  if (typeList.includes('SYSTEM') && hasSpecificPermission(systemPermissions, permission)) {
    return true;
  }
  return false;
}
